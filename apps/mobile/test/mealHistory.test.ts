...
// CanonicalFood at add time (with real energy + real confidence), but a
// HistoryItem only ever snapshotted `carbPer100g` — never energy. When the
// original food could no longer be resolved by id on "Reutilizar"/"Editar"
// (e.g. offline, or removed from the catalog), buildFallbackFood() built a
// carb-only placeholder food, so summariseMeal() reported 0 kcal and
// "unverified" confidence for a food that, in reality, has real energy and
// may be verified. HistoryItem.energyPer100g (optional, backward compatible)
// closes this gap for anything logged from now on; older persisted entries
// that lack the field are left exactly as-is (0 kcal), per CLAUDE.md's "never
...
describe("buildHistoryEntryFromLines — energyPer100g capture", () => {
  it("captures energyPer100g from a line's real energy, mirroring how carbPer100g is derived", async () => {
    const { buildHistoryEntryFromLines } = await import("../src/mealHistory");
    const entry = buildHistoryEntryFromLines([verifiedFoodLine(200)], resolveName);

    // 200g of a food with 11 kcal/100g and 1.7 g/100g carb:
    expect(entry.items[0]!.carbPer100g).toBe(1.7);
    expect(entry.items[0]!.energyPer100g).toBe(11);
  });

  it("omits energyPer100g (rather than persisting 0) when the line's amount is 0", async () => {
    const { buildHistoryEntryFromLines } = await import("../src/mealHistory");
    const entry = buildHistoryEntryFromLines([verifiedFoodLine(0)], resolveName);

    expect(entry.items[0]!.energyPer100g).toBeUndefined();
  });
  ...
  it("omits energyPer100g when the food has no energy observation at all (no fabrication)", async () => {
    ...
    const entry = buildHistoryEntryFromLines([{ food: carbOnlyFood, amount: 200 }], resolveName);
...
describe("buildFallbackFood — reconstructs energy, not just carbs", () => {
  it("adds an ENERC observation when the persisted item carries energyPer100g, so summariseMeal reports the real kcal instead of 0", async () => {
    const { buildFallbackFood } = await import("../src/mealHistory");
...
describe("end-to-end: log a verified food, lose it from the catalog, reuse the history entry offline", () => {
  it("reconstructs a fallback food whose summarised energyKcal equals the real value, not 0", async () => {
    const { buildHistoryEntryFromLines, logMeal, loadHistory, resolveHistoryEntryToLines } = await import("../src/mealHistory");
    ...
    const lines = resolveHistoryEntryToLines(loaded!, []);
...
describe("isHistoryEntry — untrusted-storage validation of energyPer100g", () => {
  it("accepts a well-formed entry with energyPer100g present", async () => {
    const { isHistoryEntry } = await import("../src/mealHistory");
    const entry = {
      id: "h1",
  ...
  it("accepts a well-formed entry with energyPer100g absent (backward compatible with pre-fix data)", async () => {
    const { isHistoryEntry } = await import("../src/mealHistory");
    const entry = {
      id: "h1",
      loggedAt: "2026-01-01T08:00:00.000Z",
  ...
  it("rejects an entry whose energyPer100g is present but not a finite number", async () => {
    const { isHistoryEntry } = await import("../src/mealHistory");
    const badEntry = {
      id: "h1",
      loggedAt: "2026-01-01T08:00:00.000Z",
    ...
    expect(isHistoryEntry(badEntry)).toBe(false);
    ...
    expect(isHistoryEntry(nanEntry)).toBe(false);
  });
});
